...
bestobj = 1000
while(prevobj - obj > stop or i < epochs ):
#while(prevobj - obj > 0):
	...
	# mini_batch_array = np.random.choice(train.shape[0],mini_batch_size, replace=False)
        # if(i%2==0):mini_batch_array = [0,1,2,3]
        # elif(i%3==0):mini_batch_array = [0,1,2,3]
        # else:mini_batch_array = [0,1,2,3]
        # print(mini_batch_array)
	prevobj = obj
	prevw = w
	prevW = W
...
#	print("dellf=",dellf)
	...
	dells = np.sum(np.dot(hidden_layer_mb[0,:],b_w)-trainlabels[mini_batch_array[0]])*b_w[0] * (hidden_layer_mb[0,0])*(1-hidden_layer_mb[0,0])*train[mini_batch_array[0]]
	dellu = np.sum(np.dot(hidden_layer_mb[0,:],b_w)-trainlabels[mini_batch_array[0]])*b_w[1] * (hidden_layer_mb[0,1])*(1-hidden_layer_mb[0,1])*train[mini_batch_array[0]]
	dellv = np.sum(np.dot(hidden_layer_mb[0,:],b_w)-trainlabels[mini_batch_array[0]])*b_w[2] * (hidden_layer_mb[0,2])*(1-hidden_layer_mb[0,2])*train[mini_batch_array[0]]
	for j in range(1, mini_batch_size):
		dells += np.sum(np.dot(hidden_layer_mb[j,:],b_w)-trainlabels[mini_batch_array[j]])*b_w[0] * (hidden_layer_mb[j,0])*(1-hidden_layer_mb[j,0])*train[mini_batch_array[j]]
		dellu += np.sum(np.dot(hidden_layer_mb[j,:],b_w)-trainlabels[mini_batch_array[j]])*b_w[1] * (hidden_layer_mb[j,1])*(1-hidden_layer_mb[j,1])*train[mini_batch_array[j]]
		dellv += np.sum(np.dot(hidden_layer_mb[j,:],b_w)-trainlabels[mini_batch_array[j]])*b_w[2] * (hidden_layer_mb[j,2])*(1-hidden_layer_mb[j,2])*train[mini_batch_array[j]]
	...
	# 	train_s = train[mini_batch_array[m]]
	# 	trainlabel_s = trainlabels[mini_batch_array[m]]
	# print(W)
	for o in range(mini_batch_size):
		train_s[o] = train[mini_batch_array[o]]
		trainlabel_s[o] = trainlabels[mini_batch_array[o]]

	#Recalculate objective
	hidden_layer_mb = np.matmul(train, np.transpose(b_W))
	# print("hidden_layer=",hidden_layer)

	hidden_layer_mb = np.array([sigmoid(xi) for xi in hidden_layer_mb])
	# print("hidden_layer=",hidden_layer)

	output_layer = (np.matmul(hidden_layer_mb, np.transpose(b_w)))
	...
	# print("Objective=",obj)
        # hidden_layer_mb = np.matmul(train_s, np.transpose(W))
        # hidden_layer_mb = np.array([sigmoid(yi) for yi in hidden_layer_mb])


        # train_s, trainlabel_s, data_s =  shuffle(data_s, mini_batch_size)
        # print(train_s)
        # print(trainlabel_s)
        # print(data_s)
        # exit()
